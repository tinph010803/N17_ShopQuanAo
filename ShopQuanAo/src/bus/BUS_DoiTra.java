...
public class BUS_DoiTra {
	...
	private DecimalFormat df = new DecimalFormat("#,###,###");
 	...
 	public void docDuLieu_CTHD(DefaultTableModel model, String mahd) {
		...
		List<ChiTietHoaDonDoiTra> dsCTTHDDT_DaDoi = DAO_ChiTietHoaDonDoiTra.getDsCTHDDT_theoMaHoaDon(mahd);
		...
		for (ChiTietHoaDon a : ds) {
			...
			for (ChiTietHoaDonDoiTra e : dsCTTHDDT_DaDoi) {
				if(a.getSanPham().getMaSanPham().equals(e.getSanPham().getMaSanPham())) {
	...
	public  void docDuLieu_CTHDDT( DefaultTableModel model, List<ChiTietHoaDonDoiTra> ds) {
		DeleteDataTable(model);
...
//			System.err.println(tien);
			...
			model.addRow(new String[] { 
					...
					sp.getMau().getValue(),
					sp.getKichThuoc().getValue(), 
					String.valueOf(e.getSoLuong()), 
					df.format(sp.getGiaBan()),
					df.format(e.tinhThanhTien()),
					df.format(tien),
	...
	public String getMa() {
		...
		String sql ="SELECT top 1* FROM HoaDonDoiTra where maHDDT like '%"+ngayhientai+"%' ORDER BY maHDDT desc";
//		System.out.println(sql);
	...
	public String updateMa() {
			String ma= getMa();
//			System.err.println("mã được truyền vào: "+ma);
