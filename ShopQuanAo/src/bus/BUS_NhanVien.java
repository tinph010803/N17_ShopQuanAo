...
public class BUS_NhanVien {
	...
	public void dodulieu(DefaultTableModel model, ResultSet rs) {
		...
		try {
			...
			while (rs.next()) {
				...
				TaiKhoan a = daotk.getTKtheoMa(rs.getString("maNhanVien")
						.trim());
				if (a == null)
					obj[9] = "Không";
				else {
	...
	public String getMa() {
		...
		ResultSet rs = ConnectionManager
				.getdata("SELECT top 1*FROM NhanVien ORDER BY maNhanVien desc");
		try {
			while (rs.next()) {
	...
	public boolean check_themNV(String ma, String ten, String sdt,
			LocalDate ngay, String email, String cccd, String diaChi) {
		...
		if (!cccd.trim().matches("\\d{12}")) {
			thongbao.thongbao("Yêu cầu nhập đúng Căn cước công dân", "Chú ý ");
			return false;
		}
		if ( !IsCheckCCCD("",cccd) ) {
	...
	public LocalDate chuyenDoiKieuNgay(JDateChooser dateChooser) {
		...
		if (selectedDate != null) {
			...
			return LocalDate.of(calendar.get(Calendar.YEAR),
					calendar.get(Calendar.MONTH) + 1, // Tháng trong Java bắt
														// đầu từ 0
	...
	public boolean IsCheckCCCD (String ma,String cccd) {
		List<NhanVien> dsNV=DAO_NhanVien.getDSNV();
		
		for (NhanVien a : dsNV) {
			if(!a.getMaNhanVien().trim().equals(ma.trim())) {
				if(a.getCccd().equals(cccd.trim()))
				return false;
			}
