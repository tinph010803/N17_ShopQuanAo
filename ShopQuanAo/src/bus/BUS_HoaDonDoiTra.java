...
public class BUS_HoaDonDoiTra {
	...
	public static void docDulieu_HDDT(DefaultTableModel model,
			List<HoaDonDoiTra> ds) {
		...
		ds.forEach(e -> {
			...
			model.addRow(new String[] { e.getMaHDDT().trim(),
	...
	public static void docDulieu_CTHDDT(DefaultTableModel model,
			List<ChiTietHoaDonDoiTra> ds) {
		DeleteDataTable(model);
		ds.forEach(e -> {
			SanPham sp = e.getSanPham();
			model.addRow(new String[] { sp.getTenSanPham().trim(),
					sp.getLoai().getValue(), sp.getMau().getValue(),
					sp.getKichThuoc().getValue(), df.format(e.getSoLuong()),
