...
public class DAO_NhanVien {
	...
	public static NhanVien layNVTheoMa(String ma) {
		ResultSet rs = ConnectionManager.getdata("select * from NhanVien where maNhanVien =N'" + ma + "'");
		NhanVien nv = null;
		...
		try {
			while (rs.next()) {
				...
				nv = new NhanVien(rs.getString("maNhanVien"), rs.getString("tenNhanVien"), rs.getString("sdt"),
						rs.getString("email"), rs.getString("diaChi"), rs.getString("cccd"), rs.getBoolean("gioiTinh"),
						a,rs.getString("hinhAnh"));
			}
			return nv;
	...
	public boolean them(NhanVien NV) {
		...
		String sql = "insert into NhanVien ( maNhanVien , tenNhanVien,gioiTinh,email, ngayVaoLam,diaChi,sdt,cccd,hinhAnh ) values ( N'"
				+ NV.getMaNhanVien() + "',N'" + NV.getTenNhanVien() + "','" + String.valueOf(i) + "',N'" + NV.getEmail()
				+ "','" + NV.getNgayVaoLam().format(formatter) + "',N'" + NV.getDiaChi() + "','" + NV.getSdt() + "','"
				+ NV.getCccd() + "','"+ NV.getHinhAnh() +"')";
	...
	public boolean sua(NhanVien NV) {
		...
		String sql =" update NhanVien set tenNhanVien = N'"+NV.getTenNhanVien()+"',gioiTinh = '"+NV.isGioiTinh()+"',email = N'"+NV.getEmail()+"',ngayVaoLam = '"+NV.getNgayVaoLam()+"',diaChi = N'"+NV.getDiaChi()+"' ,sdt = '"+NV.getSdt()+"',cccd='"+NV.getCccd()+"',hinhAnh='"+NV.getHinhAnh()+"' where maNhanVien = '"+NV.getMaNhanVien()+"'";
		int kq = ConnectionManager.executeTruyVan(sql);
		if (kq > 0) {
