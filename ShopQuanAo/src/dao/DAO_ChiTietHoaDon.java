...
public class DAO_ChiTietHoaDon {
	public static void them(entity.ChiTietHoaDon cthd) {
		String sql = "INSERT INTO [dbo].[ChiTietHoaDon] "
				...
				+ "  VALUES " + "(N'" + cthd.getSanPham().getMaSanPham()
				+ "', " + "N'" + cthd.getHoaDon().getMaHoaDon() + "', "
				+ cthd.getSoLuong() + "," + cthd.getKhuyenMai() + ","
				+ cthd.getThanhTien() + "," + cthd.getTienCuoiCung() + ")";
	...
	public static List<ChiTietHoaDon> layDSCTHDTheoMaHd(String maHD) {
		...
		ResultSet rs = ConnectionManager
				.getdata("select * from ChiTietHoaDon where ChiTietHoaDon.maHoaDon = '"
						+ maHD + "'");
		try {
			while (rs.next()) {
	...
	public static ChiTietHoaDon layCTHDtheoMaSp(String maHD, String masp) {
		...
		ResultSet rs = ConnectionManager
				.getdata("select * from ChiTietHoaDon where ChiTietHoaDon.maHoaDon = '"
						+ maHD + "' and ChiTietHoaDon.maSanPham='" + masp + "'");
		try {
			while (rs.next()) {
