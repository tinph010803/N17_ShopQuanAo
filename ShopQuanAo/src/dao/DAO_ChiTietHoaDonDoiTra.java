...
public class DAO_ChiTietHoaDonDoiTra {
	public static boolean themDsChiTietHoaDon(ChiTietHoaDonDoiTra cthddt) {
		...
		String sql = " insert into ChiTietHoaDonDoiTra (maHDDT, maSanPham,soLuong,tienTra) values ('"
				+ cthddt.getHoaDonDoiTra().getMaHDDT()
				+ "','"
				+ cthddt.getSanPham().getMaSanPham()
				+ "','"
				+ cthddt.getSoLuong() + "','" + cthddt.getTienTra() + "')";
	...
	public static List<ChiTietHoaDonDoiTra> getDsCTHDDT_theomaHDDT(String maHDDT) {
		...
		ResultSet rs = ConnectionManager
				.getdata("select * from ChiTietHoaDonDoiTra where ChiTietHoaDonDoiTra.maHDDT = '"
						+ maHDDT + "'");
		try {
			while (rs.next()) {
				...
				ChiTietHoaDonDoiTra cthddt = new ChiTietHoaDonDoiTra(sp, hddt,
						sl, tien);
				//
				ds.add(cthddt);
			}
	...
	public static List<ChiTietHoaDonDoiTra> getDsCTHDDT_theoMaHoaDon(String maHD) {
		...
		List<ChiTietHoaDonDoiTra> ds = new ArrayList<>();
		ResultSet rs = ConnectionManager
				.getdata("select c.* from HoaDonDoiTra a join HoaDon b on a.maHoaDon= b.maHoaDon join ChiTietHoaDonDoiTra c on a.maHDDT= c.maHDDT\r\n"
